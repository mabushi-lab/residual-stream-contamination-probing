...
from paths import RUNS, GENERATED, FIGURES, DATA, s
...
def mean_words(mode):
    """Average prefix length, measured from the item set rather than typed in."""
    p = s(DATA / f"contam_piqa_{mode}_suspect.jsonl")
    ...
    with open(p) as fh:
        for line in fh:
            if line.strip():
                w += len(json.loads(line)["prefix"].split())
                n += 1
    return w / max(n, 1)
...
def main():
    ...
    mac("PthreePeakHi", float(max(r["peak"] for r in rows)), ".0f")
